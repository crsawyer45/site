...
function colorVisited() {
    ...
    if (solve == true) {
        if(queue.size() == 0) {
            console.log("queue empty or unsolvable path");
            solve = false;
            return;
        }
        ...
        if(current.y > 0 && grid[current.x][current.y - 1] == unvisited) {
            queue.push(new Cell(current.x, current.y - 1, current.path));
            grid[current.x][current.y - 1] = visited;
        }
        if(current.x < grid.length - 1 && grid[current.x + 1][current.y] == unvisited) {
            queue.push(new Cell(current.x + 1, current.y, current.path));
